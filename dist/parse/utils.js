...
function getAreaByCode(code) {
  ...
  return {
    code: code,
    province: _area2.default.province_list[pCode] || "",
    city: _area2.default.city_list[cCode] || "",
    area: _area2.default.area_list[code] || ""
  };
}
...
function getTargetParentAreaListByCode(target, code) {
  ...
  result.unshift({
    code,
    name: _area2.default.area_list[code] || ""
  });
  if (["city", "province"].includes(target)) {
    code = code.slice(0, 4) + "00";
    result.unshift({
      code,
      name: _area2.default.city_list[code] || ""
    });
  }
  if (target === "province") {
    ...
    result.unshift({
      code,
      name: _area2.default.province_list[code] || ""
    });
  }
  return result;
}
...
const Reg = {
  mobile: /\b(86-[1][3-9][0-9]{9}(\-[0-9]{3,6})?)|(86[1][3-9][0-9]{9}(\-[0-9]{3,6})?)|([1][3-9][0-9]{9}(\-[0-9]{3,6})?)\b/g,
  phone: /\b(0[0-9]{2,3}-?[0-9]{7,8})\b/g,
  zipCode: /\b([0-9]{6})\b/g
};
