 ...
 */
(() => {
  const list = Array.from(document.querySelectorAll('tr')).map(el => [(el.children[1] ? el.children[1].innerText : '').trim(), (el.children[2] ? el.children[2].innerText : '').trim()]).filter(ary => ary[0] && ary[0] !== '行政区划代码');
  const updateList = [];
  const newList = [];
  /*const delList = [];
  ...
  for (const [code, name] of list) {
    const targetData = code.substr(2) === '0000' ? _area.default.province_list : code.substr(4) === '00' ? _area.default.city_list : _area.default.area_list;
    if (!targetData[code]) {
  ...
  console.log(JSON.stringify(_area.default));
