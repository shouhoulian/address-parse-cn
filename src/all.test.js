...
function addressParseTest(list = []) {
  ...
  for (const item of list) {
    ...
    if (!status) {
      isSuccess = false;
      console.log(
        "addressParseTest->fail",
        `${address} [${code}->${result.code}]`,
        result,
        results,
        options
      );
...
const list = [
  ...
  ["浙江省温州市乐清柳市镇", "330382"],
  [
    "李xx 13512222322 广西壮族自治区 桂林市 恭城瑶族自治县 恭城镇拱辰东路xx-xx号",
    "450332",
  ],
  ["李xx 13512222222 恭城 恭城镇拱辰东路08-88号", "450332"],
  [
    "四川成都高新区天府大道中段530号东方希望天祥广场a座4302号北京万商天勤(成都)律师事务所",
    "510191",
  ],
  ...
  [
    "张l,15222222222,和林格尔 盛乐经济工业园区内蒙古师范大学盛乐校区十三号楼,011500",
    "150123",
  ],
  ...
  ["山东省青岛平度市南村镇", "370283"],
  ["重庆市城口县", "500229"],
  ["重庆市云阳县双林镇桐林小学", "500235"],
  [" 上海市 浦东区 asd，大liu， ", "310115"],
  ["吐鲁番市吐鲁番宾馆", "650400"],
  ...
  [
    ...
    { code: "310120", name: "朱某某" },
  ],
  [
    "18112341234 天津大港区大港油田花园路123号 刘某某",
    { code: "120100", name: "刘某某" },
  ],
  [
    "海南省三亚市天涯区海南省三亚市天涯区凤凰镇芒果村",
    { details: "凤凰镇芒果村", code: "460204" },
  ],
  [
...
test("全国city测试解析", () => {
  ...
  console.log(
    `全国city测试解析结果 共 ${cityTestList.length} 条`,
    result2 ? "通过" : "失败"
  );
...
test("全国area测试解析", () => {
  ...
  console.log(
    `全国area测试解析结果 共 ${areaTestList.length} 条`,
    result3 ? "通过" : "失败"
  );
